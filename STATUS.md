...
**Active phase:** Phase 3 — AI-aware autoscaling (**Phase 2 CLOSED 2026-06-03**). Read `docs/phases/phase-03-autoscaling.md` to start.
**Status:** **Phase 2 COMPLETE — chat baseline live and browser-verified.** OpenWebUI → LiteLLM (Postgres-backed, **scoped virtual key**, hardened-root) → `host.docker.internal:11434` → Ollama on the RTX 4070. Owner confirmed signup + `qwen3.5` chat works in the browser (from the Windows box). All three catalog charts (`postgres`, `litellm`, `openwebui`) `deployed`; persistence + least-privilege in place; work committed and pushed to `main`. Phase 3 not yet started.
**Next action:** Begin Phase 3 — install `metrics-server`, add a CPU-based HPA on OpenWebUI as the baseline, then install **KEDA** and scale LiteLLM on an AI-relevant signal (tokens/sec or request-queue depth); drive load with `k6` and watch both HPA and KEDA react. Read `docs/phases/phase-03-autoscaling.md` first.
