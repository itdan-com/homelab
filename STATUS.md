...
**Active phase:** Phase 2 — Chat baseline as catalog-pattern Helm charts (all three charts in; only the Mac-browser step of P2.11 remains)
**Status:** P2.1–P2.10 done. **Full chat stack live in-cluster** and verified end-to-end from OpenWebUI's own pod: OpenWebUI → LiteLLM (Postgres-backed, **scoped virtual key**, hardened-root) → `host.docker.internal:11434` → Ollama on the RTX 4070 → `PONG`. Owner chose virtual keys (least-privilege) over the master key. LiteLLM is now Postgres-backed (dedicated `litellm` DB) enabling per-key auth + spend tracking; OpenWebUI holds a `qwen3.5`-scoped, revocable virtual key (SOPS-encrypted), not the admin key. `helm list -n chat` shows postgres, litellm, openwebui all `deployed`. Traefik ingress confirmed at the WSL2-host level (`curl -H 'Host: openwebui.lab.local' localhost:8080` → 200).
**Next action:** P2.11 — browser test. Quickest path (from the Windows box via RDP): add `127.0.0.1 openwebui.lab.local` to `C:\Windows\System32\drivers\etc\hosts`, browse `http://openwebui.lab.local:8080`, complete first-user signup (= admin), chat with `qwen3.5`. Mac-direct access needs Windows `netsh portproxy` or WSL2 mirrored networking (see backlog). Then **commit the catalog** (NOT yet committed — this session's work is uncommitted) and mark Phase 2 done.
**Last updated:** 2026-06-03

**GitHub remote:** https://github.com/itdan-com/homelab (private). 8 commits on `main`; this session's P2.7–P2.10 work (postgres initdb, litellm DB+hardened-root, openwebui chart, virtual-key secret) is **uncommitted** pending owner ✅.
...
- **Pin the LiteLLM image.** Still on rolling `main-stable`. openwebui is properly pinned to `v0.9.6`; do the same for litellm so a future `helm upgrade` can't silently pull a build that breaks (exactly the prisma query-engine class of surprise we hit this session). `kubectl get pod -n chat -l app.kubernetes.io/instance=litellm -o jsonpath='{.items[0].status.containerStatuses[0].imageID}'` gives today's digest.
...
- **The `openwebui` Postgres DB is currently unused.** OpenWebUI uses SQLite in its PVC (per the P2.9 spec). Either repurpose it (OpenWebUI supports a Postgres backend via `DATABASE_URL` — the cleaner stateless-frontend pattern) or drop `openwebui` from postgres `auth.extraDatabases`.
...
- **qwen3.5 burns ~200 reasoning tokens before answering.** Trivial prompts return empty `content` if `max_tokens` is stingy (Ollama's default `num_predict` can truncate mid-`<think>`). Ensure OpenWebUI/clients send generous `max_tokens`; decide whether to expose or suppress `<think>` blocks in the OpenWebUI UX.
...
- **Parameterize `_template/templates/service.yaml` port name.** Currently hardcoded to `http`; Postgres needed an inline override to `postgres`. Add `service.portName` to values.yaml so non-HTTP services don't need to fork the template. Small task, ~10 min, do during Phase 3 or whenever next touching `_template`.
- **Pin LiteLLM image to a versioned tag.** Currently `appVersion: "main-stable"` which is a rolling tag — `helm upgrade` could silently pull a new version. Once we know exactly what main-stable resolves to today, pin to that (e.g. `v1.55-stable`). `kubectl get pod -n chat -l app.kubernetes.io/instance=litellm -o jsonpath='{.items[0].status.containerStatuses[0].imageID}'` gives the digest.
