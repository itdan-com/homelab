...
**Status:** P2.1–P2.8 + P2.12 done. The LLM pipe is live end-to-end: `qwen3.5` prompt → LiteLLM (Bearer auth via SOPS-encrypted master key) → `host.docker.internal:11434` → Ollama on the RTX 4070 → response returned in OpenAI-compatible JSON. Postgres also live with persistence-survives-cluster-delete proven. Two of three Phase 2 chat-stack charts in.
**Next action:** P2.9 — write `catalog/openwebui/` chart. Point `OPENAI_API_BASE` at the in-cluster LiteLLM Service. Chat history persistence via PVC backed by the same `/homelab-data` host mount used by Postgres. Label `needs-sso: true` so Phase 5 Authentik picks it up automatically.
